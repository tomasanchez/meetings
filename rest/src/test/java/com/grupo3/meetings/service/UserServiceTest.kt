...
import com.grupo3.meetings.service.user.HashService
...
class UserServiceTest {
  ...
  private val mario = User(1.toString(), "mario@mail.com", "password")

  @Test
  fun `should create user from DTO`() {
    val dto = UserDTO("email@email.com", "password")
    val hashedPassword = "mysecretpassword"
    val expectedUser = User(1.toString(), dto.email, hashedPassword)
  ...
  fun `should fail if email is in use`() {
    ...
    given(userRepository.getByEmail(dto.email))
      .willReturn(User(3.toString(), dto.email, "other_password"))
